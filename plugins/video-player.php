<?php
	...
	$res = $db->query("SELECT * FROM tbl_video WHERE id = $id AND artist = '$artist';");
	...
	while ($row = mysqli_fetch_assoc($res)) {
		...
		$download_url = '../download/video.php?id='.$row['id'].'&amp;artist='.$row['artist'];
	}

	$poster = is_file("../files/posters/poster-$id") ? "../files/posters/poster-$id" : '../files/posters/default.png';
	$file = is_file("../files/posters/poster-$id") ? "src='$poster'" : "src='$poster' style='max-height: 197px; max-width: 50%; margin-left: 25%;'";
	$pgt = $title;
				...
				<div class="col-sm-12 col-xs-12">
					<div class="mr-player-1">
						...
						<div class="thumb-wrap">
							...
							<video src="<?php echo $path ?>" poster="<?php echo $poster ?>" controls style="background-color:#000"  class='w-full' type="<?php echo $type ?>"></video>
				...
				<div id="module3-widget-6" class="widget module3-widget row">
					<div id="module3_5a5db758ce5df">
						<div class="module3-wrap">
							<div class="row-fluid clearfix">
<?php 
	$qry = "SELECT * FROM tbl_video WHERE artist = '$artist' AND id <> $id ORDER BY created_on DESC LIMIT 6";
	...
	<div class="col-sm-6 col-xs-12">
		<div class="block6-wrap tn-block-wrap tn-category-6 clearfix">
			...
			<div class="block6-content">
				<h3 class="block-title"><a href="<?php echo $url; ?>" title="<?php echo $name; ?>"><?php echo $name; ?></a></h3><div class="block6-meta"><ul class="post-categories"><li>Music Video</li></ul><ul class="post-meta"><li class="date-post-meta"><span>|</span><time class="date updated"><?php echo $upload_on; ?></time></li></ul> </div>
			</div>
		</div>
	</div>
<?php endwhile; ?>
...
<?php include_once ("../includes/components/related_aside_vs.php") ?>
	</div>
</div>
