<?php
	...
	$res = $db->query("SELECT * FROM events WHERE id = $id AND title = '$title';");
	...
	while ($row = mysqli_fetch_assoc($res)) {
		...
		$download_url = '../download/event.php?id='.$row['id'].'&amp;title='.$row['title'];
	}
...
<div id="main-wrapper">
	...
	<div id="module4-widget-8" class="widget module4-widget">
		<div class="widget-title"><h3>GET UPDATED: <?php echo $name; ?></h3></div>
			...
			<div class="row-fluid clearfix"><!--row fluid -->
				...
				<div class="block4-content">
					<div class="block4-meta-tag">
						...
						<div class="block4-right-meta-tag"><ul class="post-meta"> <li class="comment-post-meta"><span>/</span>Trending Events</li></ul> </div>
					</div>
				</div>
...
<?php include_once ("../includes/components/related_events.php") ?>
	</div>
</div>
