<?php
	...
	$total = $db->query("SELECT count(id) as total FROM tbl_video")->fetch_assoc()['total'];
...
<?php
	$qry = "SELECT * FROM tbl_video ORDER BY created_on DESC LIMIT $start,$per_page";
	...
	while ($row=mysqli_fetch_assoc($res)):
		...
		$download = 'download/video.php?id='.$row['id'].'&amp;artist='.$row['artist'];
					...
					<div class="block2-wrap tn-block-wrap row clearfix">
						<div class="block-thumb-inner col-sm-6 col-xs-12">
							<div class="thumb-wrap">
								<a href="<?php echo $url; ?>" title="<?php echo $name; ?>" rel="bookmark"><span class="post-format"><i class="fa fa-youtube-play"></i></span><img width="320" height="180" src="<?php echo $poster; ?>" class="attachment-module_medium_thumb size-module_medium_thumb wp-post-image" alt="<?php echo $name; ?>" ></a>
							</div>
						</div>
