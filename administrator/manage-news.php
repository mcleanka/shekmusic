...
<br>
   <div class="table-responsive">
            <table class="table table-striped">
              <thead>
                <tr>
                  <th>News Title ... [Total videos:<b class='text-success'> <?php echo $num_rows; ?></b> ]</th><th>Source</th><th>Posted on:</th><th>Action</th>
                </tr>
              </thead>
              <tbody>
<?php
while ($row = mysqli_fetch_array($result)){
  ...
  echo <<< EOF
                  <tr>
                  <td><i> $name</i></td><td>$prod</td><td>$upload_on</td><td style=" text-align: center;"><a href="#"> <input type="button" onclick="updateuser($id)" class="btn btn-success btn-xs" value="Watch news"/></a> <a href="#"> <input onclick="myFunction($iduserdel)" type="button" class="btn btn-danger btn-xs" value="Delete"/></a></td>
                </tr>
EOF;
}
?>
  ...
  <script type="text/javascript">
    ...
    if (r == true) {
      window.location.href="delete-news.php?id="+x;
    } 
  }
    function updateuser(y) {
    var r = confirm("Do you want to Play this video...?");
    if (r == true) {
      window.location.href="watch-news.php?id="+y;
    } 
  }
</script>   >      
</div>
