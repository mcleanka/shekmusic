...
function uploadFile(){
	...
	if(!(filetype=='audio/mpeg'||filetype=='audio/mp3'||filetype=='audio/ogg'||filetype=='audio/wav')){
		return message("Audio Format Not Supported<br>Please Try ( MP3, WAV, OGG)");
	}
	...
	ajax.open( "POST" , "connections/parsers/audio-uploader.php" ); 
	ajax.send(formdata); 
} 
...
function completeHandler(event){ 
	...
	$("#cancelBtn").css("display","none");
	$("#progressMB").hide();
	_( "progressMB" ).innerHTML = "Audio Upload Completed Successfully";
	$("#progressMB").fadeIn('slow');
	$("#after_links").html("<a href='audio-player.php?id="+event.target.responseText+"' class='btn btn-success'>Play Audio</a><br><br><a href='audio-upload.php' class='btn btn-primary'>Upload Another</a>");
