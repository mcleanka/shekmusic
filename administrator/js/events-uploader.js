...
function uploadFile(){
	...
	var file = _( "myEvent" ).files[0];
	var poster = _("myPoster").files[0];
	...
	formdata.append( "myEvent" , file);
	formdata.append( "myPoster" , poster);
	...
	ajax.open( "POST" , "connections/parsers/events-uploader.php" ); 
	ajax.send(formdata); 
} 
...
function completeHandler(event){ 
	...
	$("#after_links").html("<a href='watch-event.php?id="+event.target.responseText+"' class='btn btn-success'>View Video</a><br><br><a href='events-upload.php' class='btn btn-primary'>Upload Another</a>");
...
function changed(){
	...
	var file = document.getElementById("myEvent").files[0];
	var x = file.name;
