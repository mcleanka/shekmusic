...
while ($row = mysqli_fetch_array($result)){
  ...
  echo <<< EOF
                  <tr>
                  <td><i> $name;</i></td><td>$prod;</td><td> $gen; </td><td style=" text-align: center;"><a href="#"> <input type="button" onclick="updateuser($id)" class="btn btn-success btn-xs" value="Play mp3"/></a> <a href="#"> <input onclick="myFunction($iduserdel)" type="button" class="btn btn-danger btn-xs" value="Delete"/></a></td>
                </tr>
EOF;
}
?>
  ...
  <script type="text/javascript">
    ...
    if (r == true) {
      window.location.href="delete-audio.php?id="+x;
    } 
  }
    function updateuser(y) {
    var r = confirm("Do you want to Play this Song...?");
    if (r == true) {
      window.location.href=".././play-audio.php?id="+y;
    } 
  }
</script> 
</div>
